...
def CreateSavedSearch(url, headers, data):
    ...
    if stdout.status_code == 201:
        ...
        msg = 'Status code was not 201 or 409 :: Responded Status code %s' % stdout.status_code
...
def DeleteSavedSearch(url, headers):
    ...
    if stdout.status_code == 200:
        module.exit_json(changed=True, responce=stdout.json())
    elif stdout.status_code == 404:
        module.exit_json(changed=False, msg='Saved Search ID does not exist')
    else:
        msg = 'Status code was not 201 or 404 :: Responded Status code %s' % stdout.status_code
...
def CreateAlertAttachment(url, headers, files):
    stdout = requests.post(url, headers=headers, files=files)
    if stdout.status_code != 201:
        module.exit_json(changed=False, responce=stdout.status_code)
    else:
        module.exit_json(changed=False, msg='Saved Search already exists!')
